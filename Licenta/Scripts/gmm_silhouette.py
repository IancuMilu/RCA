...
def gmm_silhouette_clustering(file_name, feature1, feature2, cluster_number, feature1_name, feature2_name, process_id=None):
    ...
    for i, ax in enumerate(fig_gmm.flat):
    ...
    for i, (name, group) in enumerate(grouped):
        ...
        for j in range(cluster_number):
            ...
            # Print the average silhouette score for the Gaussian Mixture model
            print(f"Gaussian Mixture Model {i}, Avg. Silhouette Score: {avg_silhouette_score:.2f}")

            # Add vertical line for average silhouette score
            axs_gmm[i // 5, i % 5].axvline(x=avg_silhouette_score, color='red', linestyle='--')
            axs_gmm[i // 5, i % 5].set_title(f'GMM {i}')

    # Save the plot
    if process_id is not None:
        print(f"Process {process_id} completed gmm silhouette clustering.")
    fig_gmm.set_size_inches(22, 11)
    fig_gmm.savefig(f'Figures/gmm_silhouette_{feature1_name}_{feature2_name}.png', dpi=100)
    print(f"Gaussian Mixture Silhouette figure saved as gmm_silhouette_{feature1_name}_{feature2_name}.png")